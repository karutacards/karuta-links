...
Hono handlers return server-rendered HTML. Dump pages have no client application. Report pages use Karuta-branded night chrome and validate Discord snowflakes, Karuta card, dye and Idol codes, and optional offense dates. The draft editor polls `/api/v1/drafts/{id}/events` while the tab is visible. Escaping is the XSS defense.
