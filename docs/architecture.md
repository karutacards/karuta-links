...
Hono handlers return server-rendered HTML. Dump pages have no client application. Report pages use Karuta-branded night chrome and validate Discord snowflakes, Karuta card, dye and Idol codes, and optional offense dates. Unsigned `GET /report` returns that HTML with Open Graph title, description and a 256-pixel lossless transparent thumbnail so Discord can unfurl the URL, then starts OAuth. `GET /report-og.webp` serves that thumbnail. It is not a favicon. The draft editor polls `/api/v1/drafts/{id}/events` while the tab is visible. Escaping is the XSS defense.
